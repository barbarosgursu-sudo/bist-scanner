...
# CONFIG
# ==========================
TEST_MODE = True           # True → Hemen çalışır | False → Saat 11:15'i bekler
RUN_HOUR = 11
RUN_MINUTE = 15
...
def get_metadata_worker(symbol):
    """Paralel çalışan zaman damgası sorgulayıcı"""
    try:
...
def fetch_snapshot_precision():
    ...
    try:
        # 1. ADIM: Tüm fiyatları topluca indir (Paralel Fiyat Çekimi)
        print(f"[{now.strftime('%H:%M:%S')}] Fiyatlar topluca indiriliyor...")
        all_data = yf.download(SYMBOLS, period="1d", group_by='ticker', threads=True, progress=False)

        # 2. ADIM: Zaman damgalarını paralel sorgula (Paralel Metadata Çekimi)
        # 20 kanaldan aynı anda zaman damgası topluyoruz
        print(f"[{now.strftime('%H:%M:%S')}] Zaman damgaları toplanıyor (Threaded)...")
        symbol_times = {}
        ...
        if all_data.empty:
            print("HATA: Yahoo veri dönmedi.")
            return None

        # 3. ADIM: Verileri Birleştir
        for symbol in SYMBOLS:
            try:
                ...
                # Eğer özel zaman damgası alınamadıysa çekilme saatini bas
...
def main():
    while True: # Railway üzerinde sürekli uyanık kalması için döngü ekledik
        now = datetime.now(ISTANBUL_TZ)

        if should_run_now(now):
            print("\n" + "="*40)
            print(f"Snapshot işlemi başlatıldı: {now.strftime('%H:%M:%S')}")
            
            payload = fetch_snapshot_precision()
            if payload:
                count = len(payload["data"])
                print(f"{count} hisse başarıyla işlendi.")
                if count > 0:
                    send_to_gas(payload)
            ...
            if TEST_MODE:
                print("Test modu aktif. 5 dakika bekleniyor...")
                time.sleep(300)
            else:
                print("Günlük görev tamamlandı. Yarın bekleniyor...")
                # Bir sonraki güne kadar uyu (yaklaşık 23 saat)
                time.sleep(82800) 
        else:
            time.sleep(60) # Saatin gelmesini beklerken her dakika kontrol et
