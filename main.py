...
def run_live_tracker(force_close=False):
    now = datetime.now(ISTANBUL_TZ)
    try:
        if force_close:
            requests.post(GAS_ENDPOINT, json={"type": "FORCE_CLOSE_ALL"}, timeout=30)
            print(f"[{now.strftime('%H:%M:%S')}] Gun sonu zorunlu kapanis sinyali gonderildi.")
            return
        ...
        if data.empty:
            print(f"HATA: yfinance veri dondurmedi!")
            return
    ...
    try:
        ...
        for symbol in SYMBOLS:
            try:
                ...
                results.append({
                    "symbol": symbol, "open": open_p, "high": high_p, 
                    "low": low_p, "last": last_p, 
...
def main():
    global EOD_DONE, LAST_DATE
    while True:
        now = datetime.now(ISTANBUL_TZ)
        today = now.date()
        
        # Gun degistiginde bayragi sifirla
        if LAST_DATE != today:
            EOD_DONE = False
            LAST_DATE = today
        ...
        if should_run_now(now):
            ...
            if payload and payload["data"]: send_to_gas(payload)
            time.sleep(60)
        elif current_total_minutes >= end_min and not EOD_DONE:
            run_live_tracker(force_close=True)
            EOD_DONE = True
            time.sleep(60)
