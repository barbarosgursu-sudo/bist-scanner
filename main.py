...
GAS_ENDPOINT = "https://script.google.com/macros/s/AKfycbwI-v7SJY-NoETn7gCts8wMPu56R6SRpfQZ3gkUqRka-znaYaMysa3hc5pCT0ttpxLo/exec"
...
def fetch_snapshot_precision():
    ...
    try:
        ...
        print(f"[{now.strftime('%H:%M:%S')}] Zaman damgalari toplaniyor...")
        symbol_times = {}
...
def main():
    while True:
        ...
        if should_run_now(now):
            print(f"\n[SCANNER] Islem baslatildi: {now.strftime('%H:%M:%S')}")
            payload = fetch_snapshot_precision()
            if payload and payload["data"]:
                print(f"{len(payload['data'])} gecerli hisse GAS'a gonderiliyor...")
                send_to_gas(payload)
            time.sleep(60)
